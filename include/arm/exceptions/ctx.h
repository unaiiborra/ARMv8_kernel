...
_Static_assert(ARM_STRUCT_ECTX_SIZE == sizeof(arm_ctx_t), "arm_ectx size mismatch");
...
_Static_assert(offsetof(arm_ctx_t, fpcr) == ARM_STRUCT_ECTX_OFFS_FPCR, "fpcr offset mismatch");

_Static_assert(offsetof(arm_ctx_t, fpsr) == ARM_STRUCT_ECTX_OFFS_FPSR, "fpsr offset mismatch");

_Static_assert(offsetof(arm_ctx_t, elr) == ARM_STRUCT_ECTX_OFFS_ELR, "elr offset mismatch");

_Static_assert(offsetof(arm_ctx_t, spsr) == ARM_STRUCT_ECTX_OFFS_SPSR, "spsr offset mismatch");

_Static_assert(offsetof(arm_ctx_t, x) == ARM_STRUCT_ECTX_OFFS_X, "x offset mismatch");

_Static_assert(offsetof(arm_ctx_t, v) == ARM_STRUCT_ECTX_OFFS_V, "v offset mismatch");

_Static_assert(ARM_STRUCT_ECTX_SIZEOF_X == sizeof(gpr_t), "arm_ectx x size mismatch");

_Static_assert(ARM_STRUCT_ECTX_SIZEOF_V == sizeof(vreg128_t), "arm_ectx v size mismatch");
...
static inline arm_spsr_m_mode ectx_get_aarch64_selected_el(const arm_ctx_t *ectx)
{
	...
	case 0b1000:
		PANIC("EL1 with SP_EL0 (EL1t) and either HCR_EL2.{NV, NV1} is {1,0} "
		      "or HCR_EL2.{NV, NV2} is {1,1}. not supported!");
	case 0b1001:
		PANIC("EL1 with SP_EL1 (EL1h) and either HCR_EL2.{NV, NV1} is {1,0} "
		      "or HCR_EL2.{NV, NV2} is {1,1}. not supported!");
	default:
...
static inline void ectx_set_aarch64_selected_el(arm_ctx_t *ectx, arm_spsr_m_mode mode)
{
	switch (mode) {
	case SPSR_EL0:
	case SPSR_EL1t:
...
static inline void
ectx_set_eret_cfg(arm_ctx_t *ectx, arm_stack stack, arm_exception_level el, arm_spsr_m_mode mode)
{
