...
thread_t *task_get_any_thread_with_sched_cpu(const task_t *task, cpuid_t sched_cpu);
...
#define DEBUG_ASSERT_TASK_IS_MEMORY_LOCKED(task)                                                   \
	DEBUG_ASSERT(task_owner_memory_is_locked(task), "task is not memory locked!")
#define DEBUG_ASSERT_TASK_IS_THREAD_LOCKED(task)                                                   \
	DEBUG_ASSERT(task_owner_threads_is_locked(task), "task is not thread locked!")
#else
