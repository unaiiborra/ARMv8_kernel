...
[[gnu::always_inline]] static inline bool spinlock_is_locked(const spinlock_t *lock)
{
...
[[gnu::always_inline]]
static inline void spinlock_release_irqrestore(spinlock_t *lock, irqflags_t irqflags)
{
...
[[gnu::always_inline]]
static inline bool spinlock_trylock_irqsave(spinlock_t *lock, irqflags_t *irqflags)
{
	*irqflags = irqsave();
...
static inline void cpulock_acquire(cpulock_t *lock)
{
	...
	if (unlikely(atomic_load_explicit(&lock->flag, memory_order_relaxed) == (int)cpuid)) {
...
{
	return atomic_load_explicit(&lock->flag, memory_order_relaxed) == (int)get_cpuid();
}
...
[[gnu::always_inline]]
static inline void cpulock_release_irqrestore(cpulock_t *lock, irqflags_t irqflags)
{
...
[[gnu::always_inline]] static inline void irqrestore_cleanup(irqflags_t *irqflags_pt)
{
...
[[gnu::always_inline]]
static inline void spinlock_release_irqrestore_cleanup(spinlock_irqrestore_cleanup_t *cleanup)
{
...
[[gnu::always_inline]]
static inline void cpulock_release_irqrestore_cleanup(cpulock_irqrestore_cleanup_t *cleanup)
{
...
#define __irqlocked_impl(id)                                                                       \
	for (irqflags_t __CONCAT(__iter_, id, _iq) = {0},                                          \
					      __DEFER(irqrestore_cleanup)                          \
						      __CONCAT(__lock_, id, _iq) = irqsave();      \
...
#define __spinlocked_irqsave_impl(id, lock)                                                        \
	for (spinlock_irqrestore_cleanup_t *                                                       \
		     __CONCAT(__iter_, id, _si) = (spinlock_irqrestore_cleanup_t *)0,              \
					   __DEFER(spinlock_release_irqrestore_cleanup)            \
						   __CONCAT(__lock_, id, _si) =                    \
							   {lock, spinlock_acquire_irqsave(lock)}; \
		     ...
		     __CONCAT(__iter_, id, _ci) = (cpulock_irqrestore_cleanup_t *)0,               \
					   __DEFER(cpulock_release_irqrestore_cleanup)             \
						   __CONCAT(__lock_, id, _ci) =                    \
							   {lock, cpulock_acquire_irqsave(lock)};  \
