...
static clock_init_t init_clocksource_and_timer(const device_t *clocksource, const device_t *timer)
{
	...
	uint32_t mult, shift, inv_mult, inv_shift;
	compute_mult_shift(cs_ops->get_freq_hz(cs_handle), &mult, &shift, &inv_mult, &inv_shift);
