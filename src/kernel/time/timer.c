...
static inline void setup_head_timer(clock_t *clock)
{
	...
	duration_ns_t abs_timer_ns =
		clock->event_list->expires - clock->offset; // get ns relative to the device

	uint64_t dev_tick = ns_to_tick(abs_timer_ns, clock->inv_mult, clock->inv_shift);
