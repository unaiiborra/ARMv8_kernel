...
)
{
	...
	if (unlikely(addr != (sysarg_t)NULL && (flags & MAP_FIXED) && (addr % PAGE_ALIGN != 0))) {
		dbg_sysc_print(
			SYSC_MMAP,
	...
	if (unlikely(
		    prot == PROT_NONE || ((prot & PROT_WRITE) && (prot & PROT_EXEC))
	    )) // wx is not supported by arm mmu
	{
	...
	spinlocked(&owner->memory_lock) {
		...
		case UREGION_OK:
			dbg_sysc_print(SYSC_MMAP, "UREGION_OK [%p %d pages]", address, pages);
			return address;
		...
		case UREGION_ERROR:
			dbg_sysc_print(SYSC_MMAP, "UREGION_ERROR (uregion_reserve error!)");
