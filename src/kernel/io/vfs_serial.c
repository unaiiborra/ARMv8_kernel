...
static void on_rx_ready(void *ctx)
{
	...
	const device_t *serial = device_get_by_uid(DEVICE_CLASS_SERIAL, device_data->dev_uid);
...
{
	const device_t *serial = device_get_by_uid(DEVICE_CLASS_SERIAL, serial_dev_uid);
	ASSERT(serial != NULL);
	DEBUG_ASSERT(serial->class_id == DEVICE_CLASS_SERIAL);
