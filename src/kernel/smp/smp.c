...
void smp_init()
{
	for (cpuid_t cpu = 0; cpu < NUM_CPUS; cpu++) {
		...
		psci_return_code code =
			psci_cpu_on((arm_cpu_affinity){.aff0 = cpu}, as_kpa((void *)_start), cpu);
...
void setup_secondary_core_el2()
{
	...
	mmu_activate_result cres = mmu_core_activate(as_kpa(mm_mmu_core_handler_get_self()));
