...
static vuintptr_t
reserve_from_fva_node(fva_node **first, fva_node *cur, fva_node *prev, vuintptr_t va, size_t bytes);
static rva_node *
push_rva(const vmalloc_lists l, size_t pages, vuintptr_t start, const char *tag, vmalloc_cfg cfg);
...
static inline vuintptr_t vunsign(vuintptr_t va)
{
	return va_zero_extend(va, MM_MMU_HI_BITS);
}

static inline vuintptr_t vsign(vuintptr_t va)
{
	return va_sign_extend(va, MM_MMU_HI_BITS);
}

// the provided va must be unsigned
static inline vmalloc_lists list_from_va(vuintptr_t uva)
{
	vuintptr_t addr_space_end = vunsign(kpa_to_kva(mm_info_mm_addr_space()));
...
safe_early void vmalloc_init()
{
	...
	*fva_dynamic_list = (fva_node){
		...
		.size = vunsign(~(vuintptr_t)0) - vunsign(kpa_to_kva(mm_info_mm_addr_space())) + 1,
	};
...
safe_early vuintptr_t vmalloc_update_memregs(const early_memreg *mregs, size_t n)
{
	...
	for (size_t i = 0; i < n; i++) {
		...
		push_rva(
			...
			mb.pages,
			va,
			is_kva((uintptr_t)mb.tag) ? mb.tag : kpa_to_kva_pt(mb.tag),
			(vmalloc_cfg){
				.kmap =
					{
						.use_kmap = true,
...
static vuintptr_t
reserve_from_fva_node(fva_node **list, fva_node *cur, fva_node *prev, vuintptr_t va, size_t bytes)
{
	DEBUG_ASSERT(
...
static vuintptr_t pop_fva(const vmalloc_lists l, size_t pages, puintptr_t p)
{
	...
	vuintptr_t kmap_va = l == KMAP_LIST ? vunsign(kpa_to_kva(p)) : (vuintptr_t)NULL;
...
static bool find_rva(vuintptr_t start, rva_node **node, rva_node **prev)
{
	start = is_kva_uintptr_t(start) ? vunsign(start) : start;
...
static size_t pop_rva(const vmalloc_lists l, vuintptr_t start, vmalloc_allocated_area_mdt *info)
{
	rva_node *node, *prev;
...
{
	DEBUG_ASSERT(
		(cfg.kmap.use_kmap && l == KMAP_LIST) || (!cfg.kmap.use_kmap && l == DYNAMIC_LIST)
	);
...
vuintptr_t vmalloc(size_t pages, const char *tag, vmalloc_cfg cfg, vmalloc_token *t)
{
	...
	if (cfg.kmap.use_kmap) {
		...
		ASSERT(cfg.assign_pa, "vmalloc: kmap can only be configured with assign_pa = true");
	}

	vuintptr_t start = pop_fva(l, pages, cfg.kmap.kmap_pa);

#ifdef DEBUG
	if (cfg.kmap.use_kmap) {
		DEBUG_ASSERT(start == vunsign(kpa_to_kva(cfg.kmap.kmap_pa)));
	} else {
		DEBUG_ASSERT(start >= vunsign(kpa_to_kva(mm_info_mm_addr_space())));
	}
#endif
...
const char *vmalloc_update_tag(vuintptr_t addr, const char *new_tag)
{
	ASSERT(addr);

	rva_node *node, *prev;

	vuintptr_t va = vunsign(addr);
...
#define T typeof(c->rva.data.reserved_nodes[0])
	...
	PANIC("vmalloc_push_pa: provided a token with an inexistent node address, it "
	      "might have been "
	      "freed before");
}
...
void vmalloc_push_pa(vmalloc_token t, size_t order, puintptr_t pa, vuintptr_t va)
{
	validate_vmalloc_token(t);
...
void vmalloc_debug_free()
{
	...
	print("\n\r");
	print("=============================================\n\r");
	print("           VMALLOC - FREE REGIONS            \n\r");
	print("=============================================\n\r");
	print("   START              END                SIZE (bytes / pages)\n\r");
	print("-------------------------------------------------------------\n\r");
	print("[K == kmapped, - == dynamic]\n\r");
	...
	print("-------------------------------------------------------------\n\r");
	printf("   TOTAL FREE: %p bytes (%p pages)\n\r", total, total / PAGE_SIZE);
	print("=============================================\n\r");
}
...
void vmalloc_debug_reserved()
{
	...
	print("\n\r");
	print("=============================================\n\r");
	print("         VMALLOC - RESERVED REGIONS\n\r");
	print("=============================================\n\r");
	print("   START              END                SIZE    FLAGS\n\r");
	print("-------------------------------------------------------------\n\r");
	...
	for (size_t i = 0; i < 2; i++) {
		...
		while (cur) {
			...
			printf("   [%p - %p)   %p B (%d p)   ", start, end, cur->size, pages);
			printf("[%c", cur->mdt.info.kmapped ? 'K' : '-');
			printf("%c", cur->mdt.info.pa_assigned ? 'P' : '-');
			printf("%c", cur->mdt.info.device_mem ? 'D' : '-');
			printf("%c]", cur->mdt.info.permanent ? '!' : '-');
	...
	print("-------------------------------------------------------------\n\r");
	printf("   TOTAL RESERVED: %p bytes (%p pages)\n\r", total, total / PAGE_SIZE);
	print("=============================================\n\r");
}
...
void vmalloc_debug_nodes()
{
	...
	print("\n\r");
	print("=============================================\n\r");
	print("           VMALLOC - RVA NODES\n\r");
	print("=============================================\n\r");

	print("---RVA CONTAINERS---\n\r");
