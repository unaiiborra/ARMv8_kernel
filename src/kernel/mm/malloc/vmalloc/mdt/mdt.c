...
{
	container_list =
		(vmalloc_mdt_container *)early_kalloc(PAGE_SIZE, "vmalloc pa mdt", true, false).va;
...
static void pa_mdt_container_free(vmalloc_mdt_container *c)
{
	...
	if (c->hdr.prev) {
		...
		PANIC("pa_mdt_container_free: prev should only be NULL if the node is "
		      "the container list, "
...
void vmalloc_pa_mdt_free(rva_node *n)
{
	...
	while (cur) {
		vmalloc_mdt_container *c = (void *)align_down((vuintptr_t)cur, PAGE_ALIGN);
