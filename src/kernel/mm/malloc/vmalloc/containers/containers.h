...
typedef struct {
	// bitfield that represents if a node is reserved (1) or free for
	// reservation (0).
...
_Static_assert(
	sizeof(fva_container_data) <= DATA_BYTES && _Alignof(fva_container_data) == DATA_ALIGN
);
_Static_assert(
	sizeof(rva_container_data) <= DATA_BYTES && _Alignof(rva_container_data) == DATA_ALIGN
);
_Static_assert(sizeof(vmalloc_container) == PAGE_SIZE && _Alignof(vmalloc_container) == PAGE_ALIGN);
