...
static void committed_init(uregion_t *region)
{
	...
	size_t bg_bytes = div_ceil(pages, BITFIELD_CAPACITY(bitfield64)) * sizeof(bitfield64);
...
)
{
	...
	bitfield64 *bf = region->pages > N ? region->committed.bg : &region->committed.sm;
...
static commited_state_e get_commited_state(uregion_t *region)
{
	...
	for (size_t w = 0; w < words; w++) {
		size_t bits_in_word =
			(w == words - 1 && region->pages % N != 0) ? region->pages % N : N;

		uint64_t mask = bits_in_word == 64 ? UINT64_MAX : (UINT64_C(1) << bits_in_word) - 1;
...
{
	uregion_node_t *node;
	rbt_find_result_e fres =
		rbt_find(&t->regions, find_region_condition, (void *)usrva, (void **)&node);
...
{
	return rbt_insert(&t->regions, node, insert_region_condition) == RBT_INSERT_OK;
}
...
)
{
	DEBUG_ASSERT(
		...
		uregion->usr_start + uregion->pages * PAGE_SIZE >= usr_va + page_count * PAGE_SIZE
	);
	...
	int32_t ires = rbt_insert(&uregion->physical_data, node, pdata_insert_condition);
...
static void
commit(uregion_t *uregion, mmu_mapping *mapping, uintptr_t usrva, uint32_t pages, bool zeroed)
{
	DEBUG_ASSERT(
...
static void physdata_destroy(void *physdata_node, void *ctx)
{
	...
	mmu_unmap_result ures =
		mmu_unmap(mapping, physdata.user_address, physdata.page_count * PAGE_SIZE, NULL);
...
)
{
	...
	for (size_t i = 0; i < kvec_len(&to_commit); i++) {
		commit(uma[i].uregion, &t->mapping, uma[i].user_address, uma[i].page_count, true);
	}

	return UREGION_ACCESS_OK;
}

static constexpr uintptr_t USR_ADDRSPACE_END = KERNEL_BASE & ~(0xFFFF000000000000ULL);
...
static bool find_free_visitor(void *node, void *ctx)
{
	...
	uintptr_t curr_end = curr->region.usr_start + curr->region.pages * PAGE_SIZE;
	...
	umemcpy_type_e type)
{
	...
	size_t off = 0;
	uintptr_t usr_start = (type == UMEMCPY_KNL_TO_USR) ? (uintptr_t)dst : (uintptr_t)src;
	...
	while (off < size) {
		...
		size_t block_remain = pnode->physdata.page_count * PAGE_SIZE - node_offset;
		size_t chunk = min(block_remain, size - off);

		void *knl_ptr = kpa_to_kva_pt(pnode->physdata.physical_address + node_offset);
...
)
{
	...
	while (off < max) {
		...
		const char *knl_ptr = kpa_to_kva_pt(pnode->physdata.physical_address + node_offset);
