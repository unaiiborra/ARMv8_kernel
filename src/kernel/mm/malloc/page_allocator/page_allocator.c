...
static inline uint8_t get_order(page_node *n)
{
	return (uint8_t)((n->node_data >> ORDER_SHIFT) & UNSHIFTED_MASK(ORDER_BITS));
}
...
bool page_allocator_get_data(puintptr_t pa, mm_page_data *data)
{
	...
	deferT(int, raw_kmalloc_unlock_defer) rklock_defer = raw_kmalloc_lock_defer();
...
safe_early void page_allocator_init()
{
	...
	size_t free_list_bytes = align_up(sizeof(uint32_t *) * MAX_ORDER, _Alignof(page_node));
