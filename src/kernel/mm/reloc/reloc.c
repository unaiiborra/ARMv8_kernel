...
extern noreturn void _jmp_to_with_offset(puintptr_t to, size_t offset, uint64_t ctx);
...
safe_early void mm_reloc(void *va_ret_addr)
{
	...
	_jmp_to_with_offset((puintptr_t)_reloc_cfg_end, KERNEL_BASE, (uint64_t)va_ret_addr);
}
...
void early_reloc_cfg_end()
{
	...
	for (size_t i = 0; i < ARRAY_LEN(EARLY_MMU_MAPPINGS); i++) {
		mmu_mapping_set_allocator(EARLY_MMU_MAPPINGS[i], as_kva(MM_STD_MMU_ALLOCATOR));
	...
	mmu_unmap(
		...
		MEM_TiB(1),
		NULL
	); // TODO: unmap from the actually reserved memory
