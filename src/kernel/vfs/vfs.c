...
_Static_assert(offsetof(file_descriptor_node_t, fd) == 24); // required by rbtree

static file_descriptor_node_t *
descriptor_new(file_descriptor_t fd, const file_operations_t *ops, void *device_data)
{
...
vfs_result_t
vfs_bind(fd_table_t *table, file_descriptor_t fd, const file_operations_t *ops, void *device_data)
{
	if (unlikely(fd < 0)) {
		return VFS_ERR_INVAL;
	}

	switch (rbt_insert_u64(&table->files, descriptor_new(fd, ops, device_data))) {
	case RBT_INSERT_OK:
		return VFS_OK;
...
vfs_result_t vfs_close(fd_table_t *table, file_descriptor_t fd)
{
	...
	vfs_result_t result = validate_descriptor_op(table, fd, OP_CLOSE, &descriptor);
...
vfs_result_t vfs_read(fd_table_t *table, file_descriptor_t fd, uint8_t *buf, uint32_t count)
{
	...
	vfs_result_t result = validate_descriptor_op(table, fd, OP_READ, &descriptor);
...
vfs_result_t vfs_write(fd_table_t *table, file_descriptor_t fd, const uint8_t *buf, uint32_t count)
{
	if ((int64_t)fd < 0 || fd > INT32_MAX) {
		return VFS_ERR_BADF;
	}
	...
	vfs_result_t result = validate_descriptor_op(table, fd, OP_WRITE, &descriptor);
...
)
{
	...
	vfs_result_t result = validate_descriptor_op(table, fd, OP_MMAP, &descriptor);

	if (unlikely(result != VFS_OK)) {
		return result;
	}

	return descriptor->ops->mmap(descriptor->device_data, addr, len, prot, flags, offset);
}
