...
monitor_data_t performance_monitor_read_fence(uint64_t cpu_freq)
{
	...
	cpuid_t cpuid = get_cpuid();
	uint64_t cycle_delta = fence_data[cpuid].cycle_end - fence_data[cpuid].cycle_start;
	uint64_t wallclock_delta =
		fence_data[cpuid].wallclock_end - fence_data[cpuid].wallclock_start;
	...
	return (monitor_data_t){
		...
		.cpu_active_percentage = cycle_delta * cntfrq * 100 / (wallclock_delta * cpu_freq),
...
monitor_data_t performance_monitor_print(uint64_t cpu_freq, const char *tag)
{
	...
	duration_ns_t wall_duration =
		clockpoint_rng_ns(data.wallclock_start, data.wallclock_end, cntfrq);
	...
	duration_ns_t cpu_duration = clockpoint_rng_ns(data.cycle_start, data.cycle_end, cpu_freq);
...
)
{
	...
	for (size_t i = 0; i < n; i++) {
		duration_ns_t wall =
			clockpoint_rng_ns(data[i].wallclock_start, data[i].wallclock_end, cntfrq);
		duration_ns_t cpu =
			clockpoint_rng_ns(data[i].cycle_start, data[i].cycle_end, cpu_freq);
	...
	for (size_t i = 0; i < n; i++) {
		...
		wall_var += (__uint128_t)(dw < 0 ? -dw : dw) * (uint64_t)(dw < 0 ? -dw : dw);
		cpu_var += (__uint128_t)(dc < 0 ? -dc : dc) * (uint64_t)(dc < 0 ? -dc : dc);
	...
	for (size_t i = 0; i < n; i++) {
		wall_sorted[i] =
			clockpoint_rng_ns(data[i].wallclock_start, data[i].wallclock_end, cntfrq);
		cpu_sorted[i] = clockpoint_rng_ns(data[i].cycle_start, data[i].cycle_end, cpu_freq);
	}
...
#define FMT_NS(v) (v) / 1000000000, ((v) / 1000000) % 1000, ((v) / 1000) % 1000, (v) % 1000
...
{
	return (duration_ns_t)((__int128_t)((int64_t)c1 - (int64_t)c0) * 1000000000 / freq);
}

duration_ns_t
clockcycle_print_profiling(clockcycle_t c0, clockcycle_t c1, uint64_t freq, const char *tag)
{
	...
	printf("[%s] %ls.%lms.%lus.%lns (cycles: %l-%l delta:%l)\n\r",
	       tag,
	       s,
	       ms,
	       us,
