...
static inline mmu_tbl tbl_from_td(const mmu_mapping *m, mmu_hw_dc dc, mmu_tbl_level l)
{
...
static inline size_t table_index(puintptr_t va, mmu_granularity g, mmu_tbl_level l)
{
...
{
	DEBUG_ASSERT((uintptr_t)tbl.dcs % g == 0);

	memzero64(tbl.dcs, tbl_entries(g) * sizeof(mmu_hw_dc));
}

static inline mmu_tbl alloc_tbl(const mmu_mapping *m, bool init_null, mmu_op_info *info)
{
	if (info) {
		info->alocated_tbls++;
	}
...
split_block(const mmu_mapping *m, mmu_tbl parent, size_t index, mmu_tbl_level l, mmu_op_info *info)
{
...
static inline void free_tbl(const mmu_mapping *m, mmu_tbl tbl, mmu_tbl_level l, mmu_op_info *info)
{
