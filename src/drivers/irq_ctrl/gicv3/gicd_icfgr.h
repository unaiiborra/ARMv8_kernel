...
static inline uint8_t GICV3_GICD_ICFGR_get(const GICD_ICFGR_VALUE_STRUCT_NAME r, size_t slot)
{
	if (slot > 15) {
...
static inline void GICV3_GICD_ICFGR_set(GICD_ICFGR_VALUE_STRUCT_NAME *r, size_t slot, uint8_t v)
{
	if (slot > 15) {
		PANIC("GICD_ICFGR: slot must be <= 15");
	}
