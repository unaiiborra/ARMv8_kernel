...
static int32_t gicv3_irq_set_priority(driver_handle_t handle, uint32_t irq, uint8_t priority)
{
	if (irq < 32) {
		uint32_t rd = arm_get_cpu_affinity_as_u32();
		uint32_t n = irq / 4;
...
static int32_t
gicv3_irq_set_trigger(driver_handle_t handle, uint32_t irq, irq_ctrl_ops_trigger_t trigger)
{
	if (irq < 32) {
		if (irq < 16) {
...
static int32_t gicv3_irq_set_target(driver_handle_t handle, uint32_t irq, uint32_t target_cpu)
{
	if (irq < 32) {
