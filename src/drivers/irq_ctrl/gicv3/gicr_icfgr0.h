...
{
	return (GicrIcfgr){.val = *((reg32_ptr)(GICV3_SGI_BASE(base, cpu) + GICR_ICFGR0_OFFSET))};
}

static inline void GICV3_GICR_ICFGR0_write(uintptr_t base, uint32_t cpu, GicrIcfgr r)
{
...
{
	return (GicrIcfgr){.val = *((reg32_ptr)(GICV3_SGI_BASE(base, cpu) + GICR_ICFGR1_OFFSET))};
}

static inline void GICV3_GICR_ICFGR1_write(uintptr_t base, uint32_t cpu, GicrIcfgr r)
{
	*((reg32_ptr)(GICV3_SGI_BASE(base, cpu) + GICR_ICFGR1_OFFSET)) = r.val;
}

static inline void GICV3_GICR_ICFGR_set(GicrIcfgr *r, uint32_t slot, uint32_t val)
{
	if (slot > 15) {
