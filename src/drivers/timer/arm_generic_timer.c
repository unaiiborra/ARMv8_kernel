...
const clocksource_ops_t *const ARM_GENERIC_TIMER_CLOCKSOURCE_OPS = &CLOCKSOURCE_OPS;
...
static void timer_irq_handle(driver_handle_t handle)
{
	...
	DEBUG_ASSERT(
		(sysreg_read(CNTV_CTL_EL0) & (ISTATUS(true) | IMASK(false) | IENABLE(true))) ==
		(ISTATUS(true) | IMASK(false) | IENABLE(true))
	);

	sysreg_write(CNTV_CTL_EL0,
