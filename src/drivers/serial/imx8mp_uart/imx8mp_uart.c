...
static int32_t imx8mp_uart_set_baud(driver_handle_t handle, uint32_t baud, uint32_t clock_hz)
{
	if (baud == 0) {
		return -1;
	}
...
#define SET_SRC(bit, status)                                                                       \
	sources |=                                                                                 \
		((bitfield32)(((uint32_t)(status) & (uint32_t)bitfield_get(state->irq_cache, bit)) \
			      << (bit)))
